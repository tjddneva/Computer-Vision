...
def create_corner_response_matrix(img, sigma, k, thresh):
    ...
    # w(x, y) * (Ix_square)
    Ix_2_win = convolve2d(Ix*Ix, win, mode='same',
                          boundary='fill', fillvalue=0)
    # w(x, y) * (Iy_square)
    Iy_2_win = convolve2d(Iy*Iy, win, mode='same',
                          boundary='fill', fillvalue=0)
    # w(x, y) * (Ix times Iy)
    Ix_Iy_win = convolve2d(Ix*Iy, win, mode='same',
                           boundary='fill', fillvalue=0)
...
def plot_corners(img_clr, corners):
    ...
    for i in range(corners.shape[0]):
        cv2.circle(img_clr, (corners[i, 0], corners[i, 1]), radius=3, color=[
                   0, 255, 0], thickness=-1)
...
def local_non_maximal_suppression(corner_resp, kernel_size=25):
    ...
    # Pad for window
    corner_resp = np.pad(
        corner_resp, ((pad, pad), (pad, pad)), mode="constant")
    ...
    for i in range(pad, h-pad):
        for j in range(pad, w-pad):
            max_val = np.amax(
                corner_resp[i - pad: i + pad + 1, j - pad: j + pad + 1])
...
def get_max_corners(corner_resp, max_no_corners=-1):
    ...
    # num_corners x 2 (col_1 is x coord, col_2 is y_coord)
...
def detect_corners(img_path, nms_kernel_size, sigma, k=0.06, thresh=0.75, max_no_corners=-1):
    ...
    nms_corner_resp = local_non_maximal_suppression(
        corner_response, nms_kernel_size)
...
if __name__ == "__main__":
    ...
    imgs = ["KakaoTalk_20211123_010846968.png"]
    for img_path in imgs:
        print("#########")
        lst_sigma = [0.707, 1, 1.414, 2, 2.1, 2.7, 3]
        # lst_sigma = [0.707]
        nms_kernel_size = 25  # local maximum depression 이랑 관련
        k = 0.06
        thresh = 1
        max_no_corners = 500

        for sigma in lst_sigma:
            detect_corners(img_path, nms_kernel_size,
                           sigma, k, thresh, max_no_corners)
